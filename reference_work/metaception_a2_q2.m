...
for i=2:size(t_laser,1)
    ...
    for n=1:nparticles
        ...
        for beam=1:2         
            ...
            % Check if the laser measurement is valid
            if isnan(y_laser(i, j)) || y_laser(i, j) > y_laser_max
                continue
            end
            
            % Laser value in world frame            
            x_turtle =  y_laser(i, j) * cos(angles(j)) - 0.1;
            y_turtle =  y_laser(i, j) * sin(angles(j));
            % Make into homogeneous vector
            laser = [x_turtle;   y_turtle;   1];
            % Rigid transform to world frame
            R = [   cos(theta_interp(i))    -sin(theta_interp(i))   x_interp(i);
                    sin(theta_interp(i))    cos(theta_interp(i))    y_interp(i);
                    0                       0                       1];
            laser = R * laser;
            % Convert to MATLAB frame
            x_l =   round((laser(1)-ogxmin)/ogres);
            y_l =   round((laser(2)-ogymin)/ogres);
            
            
            % Particle laser prediction
            H = T * [   cos(angles(j))  -sin(angles(j)) -0.1;
            ...
            % To occupancy grid frame
            x_pl =   min(300, max(1, (x_pl-ogxmin)/ogres));
            y_pl =   min(180, max(1, (y_pl-ogymin)/ogres));
            
            % Trace laser scan angle to calculate the expected range value
            for incr = r_min_laser:(ogres/2):y_laser_max
                % Stop if the laser hit an obstacle
                if (oglo(round(y_pl), round(x_pl)) >= 0)
                    continue
                end
                
                % Increment along ray
                x_pl =  max(1, min(300, x_pl + incr*cos(theta_pl)));
                y_pl =  max(1, min(180, y_pl + incr*sin(theta_pl)));
            end
            
            % Calculate and update weights, assuming Gaussian distribution
            gauss =         (1/sqrt(2*pi*laser_var))*exp(-norm([x_pl; y_pl]-[x_l; y_l])^2/(2*laser_var));
